...
class ODriveInterfaceAPI(object):
    ...
    def __init__(self, calibrate_axis=1, USER_CPR = 4000, drive_mode = "velocity", logger=None):
	self.calibrate_axis = calibrate_axis
	self.user_cpr = USER_CPR
	...
	# return True
	    ...
	    while abs(axis.encoder.vel_estimate) > 0.01:
		time.sleep(1)
	    self.logger.info("Motor Calibrated")
    ...
    def drive(self, left_motor_val, right_motor_val, pos):
        if not self.driver:
            self.logger.error("Not connected.")
            return
	...
	if self.drive_mode == "velocity":
	    ...
	    self.logger.info("Motor Drived with Position Setpoint %d" % pos)
	    if self.calibrate_axis != 1:
		self.left_axis.controller.pos_setpoint = pos
	    else:
            	self.right_axis.controller.pos_setpoint = pos
