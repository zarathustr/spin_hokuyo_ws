...
class ODriveNode(object):
	...
	if self.publish_joint_state:
	    ...
	    self.RADIANS_PER_ENCODER_TICK = 2.0 * 3.1415926 / self.user_cpr
    ...
    def connect_driver(self, request):
        ...
        self.driver = ODriveInterfaceAPI(calibrate_axis=self.Calibrate_Axis, USER_CPR = self.user_cpr, drive_mode = self.drive_mode, logger=ROSLogger())
	...
	if self.drive_mode != "velocity":
        ...
        self.driver.drive(left_linear_val, right_linear_val, pos)
	...
	if self.publish_joint_state:
	    if self.Calibrate_Axis != 1:
	        self.state.position = new_pos_l
	        self.state.speed = vel_l
	    else:
